...
- D
  - `Depurar (Debugging)` : Proceso de identificar, asilar y corregir errores (bugs) en el código. 
- E
- F
...
- K
- L
  - `Libreria (Package)` : Código reutilizable agrupado en funciones, clases o scripts. También denominado **módulo**.
- M
- N
- O
  - `Output` : Valor de salida.
- P
  - `Prueba unitaria (Unit test)` : Prueba automatizada que valida el comportamiento de una unidad mínima de código, como una función o un método.
- K
- R
  - `Recursión` : Técnica donde una función se llama a sí misma.
  - `Refactorizar` : Reescribir partes del código para mejorar su estructura sin cambiar su funcionalidad.
- S
  - `Script` :  Archivo con código Python que se ejecuta desde el principio hasta el final.
- T
  - `Traceback` : Mensaje de error que aparece en Python cuando una excepción no es capturada. Incluye el archivo, línea y tipo de error.
- U
- V
  - `Variable` : Nombre que guarda un valor en memoria. El valor puede cambiar durante la ejecución del programa.
- W
- X
- Y
- X
...
- `,` : Coma
- `;` : Punto y coma
- `/` : Barra, Barra lateral o Slash
- `()` : Paréntesis
- `[]` : Corchetes o Brackets
- `{}` : Llaves o Braces
- `&` : Ampersand
- `@` : Arroba
- `#` : Almohadilla o Hash
- `~` : Virgulilla o Tilde de ñ
- `"` : Comillas dobles
- `'` : Comillas simples
- `^` : Circunflejo
